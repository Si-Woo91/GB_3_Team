...
<body>
	<section>
		<div class="content">
			<div id="catg">
				<h2>
					<%= goodsList.get(0).getgCatg()%></h2>
				<hr />

				<% for (goodsDTO g : goodsList ){ %>
				<li class="items" border="1" align="center">
					<!-- gID값을 상세페이지로 넘겨줌 --> <a class="itemlink"
					href="${pageContext.request.contextPath }/goods/goodsDetail.goods?gID=<%= g.getgID() %>">
						<img
						src="${pageContext.request.contextPath }/img/<%=foldername %>/<%= g.getgImgs()%>"
						class="img_prod" width="300" height="300"><br> <span
						class="name"><%= g.getgName() %></span><br> <span
						class="price"><%= df.format(g.getgPRICE()) %> 원</span><br>
				</a>
				</li>
				<% } %>
