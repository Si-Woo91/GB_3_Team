...
<body id="payment">
	<section id='cart'>
		...
		</span> <input type="button" class="select_button" id="select_button"
			value=" 선택상품 삭제  " onclick="deleteTableRow();"> <br>
		<div class="align_center">
			<input type="button" class="order_button1" id="order_button1"
				value="선택주문" onclick="jumpPageSome();"> &nbsp; <input
				type="button" class="order_button2" id="order_button2" value="전체주문"
				onclick="jumpPageAll()">
		</div>
...
<%}%>
	...
	function deleteTableRow() {
		...
		if (count == 0) {
			...
			xhr.open("GET",
					'${pageContext.request.contextPath }/payment/cartdel.pay?delcids='
							+ del_cids, true);
			xhr.send();
	...
	function jumpPageSome() {
		...
		if (checked == 0) {
			...
			for (let i = 0; i < cid_arr.length; i++) {
				if (checks[i].checked == true)
					cid_arr_checked.push(cid_arr[i]);
			}
			location.href = '${pageContext.request.contextPath }/payment/checkout.pay?cids='
					+ cid_arr_checked;
		}
	}
